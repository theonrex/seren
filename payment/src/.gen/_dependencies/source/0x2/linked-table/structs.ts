...
export function isLinkedTable(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::linked_table::LinkedTable` + '<'); }
...
export function isNode(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::linked_table::Node` + '<'); }
