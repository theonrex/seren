...
export function isDisplay(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::display::Display` + '<'); }
...
export function isDisplayCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::display::DisplayCreated` + '<'); }
...
export class DisplayCreated<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V29}::display::DisplayCreated`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = DisplayCreated.$typeName; readonly $fullTypeName: `${typeof PKG_V29}::display::DisplayCreated<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = DisplayCreated.$isPhantom;

 readonly id: ToField<ID>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: DisplayCreatedFields<T>, ) { this.$fullTypeName = composeSuiType( DisplayCreated.$typeName, ...typeArgs ) as `${typeof PKG_V29}::display::DisplayCreated<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): DisplayCreatedReified<ToPhantomTypeArgument<T>> { return { typeName: DisplayCreated.$typeName, fullTypeName: composeSuiType( DisplayCreated.$typeName, ...[extractType(T)] ) as `${typeof PKG_V29}::display::DisplayCreated<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: DisplayCreated.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => DisplayCreated.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DisplayCreated.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => DisplayCreated.fromBcs( T, data, ), bcs: DisplayCreated.bcs, fromJSONField: (field: any) => DisplayCreated.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => DisplayCreated.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => DisplayCreated.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => DisplayCreated.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => DisplayCreated.fetch( client, T, id, ), new: ( fields: DisplayCreatedFields<ToPhantomTypeArgument<T>>, ) => { return new DisplayCreated( [extractType(T)], fields ) }, kind: "StructClassReified", } }
...
export function isVersionUpdated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::display::VersionUpdated` + '<'); }
